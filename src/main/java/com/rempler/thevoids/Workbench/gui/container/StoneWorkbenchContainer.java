package com.rempler.thevoids.Workbench.gui.container;
...
import com.rempler.thevoids.Workbench.gui.slots.SlotFastCraft;
import com.rempler.thevoids.Workbench.gui.slots.WrapperSlot;
...
import com.rempler.thevoids.Workbench.network.S2CLastRecipePacket;
...
public class StoneWorkbenchContainer extends Container {
    ...
    public final StoneWorkbenchCraftingInventory craftMatrix;
    public final CraftResultInventory craftResult = new CraftResultInventory();
    public final World world;
    public final StoneWorkbenchTileEntity tileEntity;
    ...
    public StoneWorkbenchContainer(int id, PlayerInventory inv, World world, BlockPos pos) {
        super(StoneWorkbench.Objects.stone_workbench_container, id);

        this.world = world;
        this.player = inv.player;
        this.tileEntity = (StoneWorkbenchTileEntity) getTileEntityAtPos(pos, world);
        currentContainer = tileEntity.currentContainer;
        this.craftMatrix = new StoneWorkbenchCraftingInventory(this, tileEntity.input);
        ...
        for (Direction dir : Direction.values()) {
            ...
            if (te != null && !(te instanceof StoneWorkbenchTileEntity)) {
        ...
        if (!tileEntities.isEmpty()) {
            addSideContainerSlots(tileEntities, -125, 17);
        }
        addPlayerSlots(inv);
    ...
    public static IRecipe<CraftingInventory> findRecipe(CraftingInventory inv, World world, PlayerEntity player) {
        /*return world.getRecipeManager().getRecipes(IRecipeType.CRAFTING).values().stream().flatMap(recipe -> {
            try {
                ...
                StoneWorkbench.LOGGER.error("Bad recipe found: " + recipe.getId().toString());
                StoneWorkbench.LOGGER.error(e.getMessage());
    ...
    private void addSideContainerSlots(List<TileEntity> tes, int xPos, int yPos) {
    ...
    //clientside only
    @Override
    public void setAll(@Nonnull List<ItemStack> p_190896_1_) {
    ...
    private void syncRecipeToAllOpenWindows(final IRecipe<CraftingInventory> lastRecipe, List<ServerPlayerEntity> players) {
        players.forEach(otherPlayer -> {
            ...
            ((StoneWorkbenchContainer) otherPlayer.openContainer).lastRecipe = lastRecipe;
    ...
    private List<ServerPlayerEntity> getAllPlayersWithThisContainerOpen(StoneWorkbenchContainer container, ServerWorld server) {
        return server.getPlayers().stream()
                .filter(player -> hasSameContainerOpen(container, player))
                .collect(Collectors.toList());
    }

    private boolean hasSameContainerOpen(StoneWorkbenchContainer container, PlayerEntity playerToCheck) {
        return playerToCheck instanceof ServerPlayerEntity &&
                ...
                this.sameGui((StoneWorkbenchContainer) playerToCheck.openContainer);
    }

    public boolean sameGui(StoneWorkbenchContainer otherContainer) {
